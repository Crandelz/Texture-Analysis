/**
 * Created by Angel Zeitoune on 06/04/2016.
 * https://github.com/Crandelz/Texture-Analysis/
 *
 * Changes:
 *	- Separation of fiji visual interface of GLCM calculation
 *	- Fixed GLCM calculation matrix (important). Small difference, but more accuracy
 * 	- Fixed correlation calculation (very important)
 *	- On multi channel images like (lsm), can be selected to calculate over selected channel or all channels
 *  - Result table is not cleaned (close it to delete old results)
 *  - Angle and Offset (step) are showed in result table
 */
...
public class Gray_Level_Cooccurrence_Matrix implements PlugInFilter {
    ...
    public void writeToResultsTable(boolean ShowResults){
        ...
        rt.incrementCounter();
		rt.setValue("Angle", row, gl.phi);
		rt.setValue("Offset", row, gl.pixelOffset);
